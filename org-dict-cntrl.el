...
(defun org-dict-cntrl--replace-exemples (dom)
  ;; span.tlf_cexemple
...
(defun org-dict-cntrl--remove-cdevette (dom)
...
(defun org-dict-cntrl--replace-numbering (dom)
...
(defun org-dict-cntrl--remove-italic (dom)
...
(defun org-dict-cntrl--replace-scripts (dom)
...
(defun org-dict-cntrl--node-replace-with-surround (dom node char)
...
(defun org-dict-cntrl--replace-markup (dom)
  ...
  (let* ((all-bold-nodes (org-dict--dom-select dom '((:tag b))))
    ...
    (mapc (lambda (node)
	   (org-dict-cntrl--node-replace-with-surround dom node "*"))
	   target-bold-nodes)
    (mapc (lambda (node)
	   (org-dict-cntrl--node-replace-with-surround dom node "*"))
...
(defvar org-dict-cntrl--cplan-regexps '("^[0-9].*Section\\.$"
					"^\\([I]\\{1,3\\}\\|[I]?V\\|V[I]\\{1,3\\}\\|[I]?X\\)\\.[ ]?−$"
					"^[A-H]\\.[ ]?−$" "^[0-9]\\.[ ]?$" "^[a-z])$"
					"[αβγδϵζηθικλ])"))

(defun org-dict-cntrl--parse-cplan (string)
  "Parse TLFi numbering STRING. Return its associated level."
  (cl-loop with regexps = org-dict-cntrl--cplan-regexps
	   with level = 0
...
(defun org-dict-cntrl--space-or-newline (node)
...
(defun org-dict-cntrl--create-section-title (title current-depth numbering)
...
(defun org-dict-cntrl--parse-parah (parah-node)
  ...
  (cl-loop for node in (dom-children parah-node)
	   ...
	   if (stringp node)
	       ...
	       and collect (org-dict-cntrl--create-section-title title current-depth level-numbering)
	       and collect (apply #'concat (flatten-tree (org-dict-cntrl--parse-parah node)))
	       and do (setq level-numbering (1+ level-numbering))
	        ...
	        do (setq current-depth (org-dict-cntrl--parse-cplan (dom-texts node "")))
		    ...
		    collect (org-dict-cntrl--create-section-title title current-depth level-numbering)
		end and
		collect (apply #'concat (flatten-tree (dom-texts node ""))) and
		collect (org-dict-cntrl--space-or-newline node)
	   end))
...
(defun org-dict-cntrl--parse-entry-content (dom)
  (let ((article-node (org-dict--dom-select dom ;; #lexicontent > .div
							...
							(> (:tag div))))))
  ;; (org-dict-cntrl--remove-cdevette article-node)
  (org-dict-cntrl--replace-markup article-node)
  (org-dict-cntrl--remove-italic article-node)
  (org-dict-cntrl--replace-exemples article-node)
  (org-dict-cntrl--replace-scripts article-node)
  ;; (org-dict-cntrl--replace-numbering article-node)
  (apply #'concat
	 (flatten-tree
		   ...
		   if (org-dict--dom-node-simple-selector-p '(:tag div :attrs ((class . "tlf_parah"))) node)
		     ;; do (mapc (lambda (child) (message "node:'%s', class:%s" (if (stringp child) child (car child)) (when (not (stringp child)) (org-dict--dom-node-class child)))) (dom-children node)) and
	               ;; do (message "node:%s, class:%s, level-numbering:%s" (car node) (org-dict--dom-node-class node) top-level-numbering) and
                       collect (org-dict-cntrl--parse-parah node) and
...
(defun org-dict-cntrl--parse-entry (dom)
  ...
  (let ((entry-name (org-dict-cntrl--current-entry-name dom)))
	...
	(org-dict-cntrl--parse-entry-content dom)))
      (insert "\n")
      ;; TODO add etymology
